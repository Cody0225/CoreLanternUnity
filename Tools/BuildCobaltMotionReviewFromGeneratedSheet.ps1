...
$OutDir = Join-Path $ProjectRoot "Assets\ArtSource\CobaltPixelAnim_20260601\motion_review_20260601"
New-Item -ItemType Directory -Force -Path $OutDir | Out-Null

$ChromaCopy = Join-Path $OutDir "CobaltPup_MOTION_REVIEW_v1_sheet_chromakey.png"
$CleanSource = Join-Path $OutDir "CobaltPup_MOTION_REVIEW_v1_sheet_transparent_clean.png"
...
transparent_sheet = out_dir / "CobaltPup_MOTION_REVIEW_v1_sheet_transparent.png"
...
for i, fr in enumerate(run_frames, 1):
    fr.save(frames_dir / f"CobaltPup_MOTION_REVIEW_v1_RUN_{i:02}.png")
for i, fr in enumerate(attack_frames, 1):
    fr.save(frames_dir / f"CobaltPup_MOTION_REVIEW_v1_ATTACK_{i:02}.png")

def render_gif(frames, out_name, label, duration=95, canvas=(420, 300), mode="center"):
    rendered = []
    for fr in frames:
...
render_gif(run_frames, "CobaltPup_MOTION_REVIEW_v1_RUN.gif", "MOTION REVIEW / RUN", 90)
render_gif(attack_frames, "CobaltPup_MOTION_REVIEW_v1_ATTACK.gif", "MOTION REVIEW / ATTACK - muzzle origin", 110, canvas=(560, 300), mode="left")

def shrink_check():
    samples = [run_frames[1], run_frames[3], attack_frames[1], attack_frames[2]]
    labels = ["RUN push A", "RUN push B", "ATK charge", "ATK fire"]
    sizes = [64, 96, 128]
    ...
    sheet.save(out_dir / "CobaltPup_MOTION_REVIEW_v1_shrink_check.png")

def before_after():
    old_sheet = project_root / "Assets" / "ArtSource" / "CobaltPixelAnim_20260601" / "CobaltPup_PixelAnimSheet_transparent.png"
    ...
    old_samples = [
        crop_old((78, 318, 292, 505)),
        crop_old((548, 318, 765, 505)),
        crop_old((424, 558, 655, 750)),
        crop_old((690, 558, 1086, 750)),
    ]
    ...
    d.text((16, 12), "BEFORE: AI rough crop / AFTER: new motion review", font=font, fill=TEXT)
    for c in range(4):
        d.text((c * cell_w + 16, 42), labels[c], font=small, fill=TEXT)
        for r, sample in enumerate([old_samples[c], new_samples[c]]):
    ...
    canvas.save(out_dir / "CobaltPup_MOTION_REVIEW_v1_before_after.png")
...
print("run_gif=" + str(out_dir / "CobaltPup_MOTION_REVIEW_v1_RUN.gif"))
print("attack_gif=" + str(out_dir / "CobaltPup_MOTION_REVIEW_v1_ATTACK.gif"))
print("shrink=" + str(out_dir / "CobaltPup_MOTION_REVIEW_v1_shrink_check.png"))
print("before_after=" + str(out_dir / "CobaltPup_MOTION_REVIEW_v1_before_after.png"))
'@
...
$tmpScript = Join-Path ([System.IO.Path]::GetTempPath()) "build_cobalt_motion_review.py"
