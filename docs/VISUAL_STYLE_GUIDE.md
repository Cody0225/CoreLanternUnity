...
### 11-2. モーション方針＝多パーツ・コード制御リグ
- **目的**: 「絵が揺れるだけ」ではなく、**表情変化・脚の歩行・被弾の体勢変化**で没入感を出す。
- **方式**: キャラを **体／頭／耳／前脚×2／後脚×2／尾／顔（通常・ダメージ）** の**別パーツPNG**で生成 → **コードで多パーツ化**して動かす（このゲームは絵をコードで組む構造のため、Spine等より相性が良い）。
- **流用**: 既存の `playerMotionPhase`（歩行位相）・`playerHitPulse`（被弾）信号でパーツを駆動できる。
- **進め方**: まず **S1 で1体だけ試作**（パーツ生成→リグ実装→検証）→ OKなら仕様化して他へ展開。**全キャラ一括再生成はしない**（手戻り防止）。
- **既存の `enhancedVisuals` 手続き演出**（1枚絵のバウンド/スクワッシュ/のけぞり）は没入感不足。**リグの土台信号としてのみ残す**（破棄しない）。
- **TECH_FIX注意**: パーツは「位置を保ったまま背景だけ抜く」処理が必要（通常の切り抜き＋中央寄せだと**位置合わせが壊れる**）。リグ着手時に対応。
...
- S1 リグ用パーツ生成ブリーフ（Codexレート制限明け後に実行。未doc化）
