...
- **DETAIL LOCK（2026-06-16 追加）**: 表情4枚で「**顔以外（角・角の色・装甲色・発光色・模様・スパイク・翼）は完全に同一**」。変えるのは顔(目/口)だけ。独立生成だと細部（特に角の色）がズレるのでプロンプトで明示強制する。
- **EVOLVE表情は段階で変える（2026-06-16）**: L0/L1＝興奮気味/かわいいでOK。**L2/L3以降のEVOLVEは「優しい/かわいい/ニコニコ」NG**＝獰猛・力に目覚める・凛々しい強さにする。
- **透過＝全域マゼンタキー**: 境界連結のみだと、ポーズで体に囲まれたマゼンタ隙間が残り桃シミになる。マゼンタはキャラ色に無いので**全域で抜く**。ツール `Tools/make_transparent.py`（2026-06-16 全域キー化済）。
- **処理実行の注意**: 日本語パス対策で `py -X utf8` ＋ ASCII作業ディレクトリから実行（FS encoding が cp932 に振れると `デスクトップ` が mojibake で file-not-found になる）。
